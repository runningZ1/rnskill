#!/usr/bin/env python3
"""
Fetch WeChat public account article with video channel (视频号) support.

Enhanced version that can download videos from video channels using
the online parsing service: https://sph.litao.workers.dev/

Based on:
- Original: Pluviobyte/rnskill
- Video channel download: ltaoo/wx_channels_download

Usage:
    python fetch_wechat_with_channels.py <url> [--output-dir DIR] [--raw]
"""
...
# Video channel parsing service
SPH_PARSE_URL = "https://sph.litao.workers.dev/"
...
def parse_sph_link(sph_url: str) -> dict:
    """Parse video channel link using online service."""
    try:
        # The online service expects POST with URL
        data = json.dumps({"url": sph_url}).encode('utf-8')
        req = urllib.request.Request(
            SPH_PARSE_URL + "api/parse",
            data=data,
            headers={
        ...
        print(f"  ⚠ Failed to parse sph link: {e}", file=sys.stderr)
        return None
...
def extract_videos_from_html(html: str) -> tuple:
    """Extract video URLs and video channel info from HTML."""
    videos = []
    video_channels = []
    ...
    # Pattern 3: Video channel wxv_ IDs
    wxv_pattern = r"video_id:\s*['\"](wxv_[^['\"]+)['\"]"
    wxv_ids = re.findall(wxv_pattern, html)
    for wxv_id in wxv_ids:
        video_channels.append({"type": "wxv", "id": wxv_id})
    
    # Pattern 4: Finder usernames
    finder_pattern = r"username\\x22:\\x22([^@]+@finder)\\x22"
    finder_usernames = re.findall(finder_pattern, html)
    for username in finder_usernames:
        video_channels.append({"type": "finder", "username": username})
    
    # Pattern 5: Video channel share links (weixin.qq.com/sph/xxx)
    sph_pattern = r'https?://weixin\.qq\.com/sph/([A-Za-z0-9]+)'
    sph_ids = re.findall(sph_pattern, html)
    for sph_id in sph_ids:
        video_channels.append({"type": "sph", "url": f"https://weixin.qq.com/sph/{sph_id}"})
    
    # Pattern 6: Finder video URLs
    finder_video_pattern = r"findermp\.video\.qq\.com[^\"]*"
    finder_video_urls = re.findall(finder_video_pattern, html)
    for url in finder_video_urls:
        video_channels.append({"type": "finder_url", "url": f"https://{url}"})
    
    return videos, video_channels
...
def main():
    ...
    # Extract videos and video channels
    print("Extracting videos...", file=sys.stderr)
    video_urls, video_channels = extract_videos_from_html(html)
    video_count = 0
    video_map = {}
    
    # Report video channel info
    if video_channels:
        print(f"Found {len(video_channels)} video channel items:", file=sys.stderr)
        for vc in video_channels:
            if vc["type"] == "wxv":
                print(f"  - wxv ID: {vc['id']}", file=sys.stderr)
            elif vc["type"] == "finder":
                print(f"  - Finder username: {vc['username']}", file=sys.stderr)
            elif vc["type"] == "sph":
                print(f"  - Share link: {vc['url']}", file=sys.stderr)
            elif vc["type"] == "finder_url":
                print(f"  - Finder URL (may be expired): {vc['url'][:50]}...", file=sys.stderr)
    
    # Try to download direct video URLs
    if video_urls:
    ...
    # Try to parse video channel share links
    sph_links = [vc for vc in video_channels if vc["type"] == "sph"]
    if sph_links:
        print(f"\nFound {len(sph_links)} video channel share links", file=sys.stderr)
        print("Note: Video channel videos require the online parsing service", file=sys.stderr)
        print(f"Service: {SPH_PARSE_URL}", file=sys.stderr)
        
        for sph in sph_links:
            print(f"\n  Share link: {sph['url']}", file=sys.stderr)
            print(f"  To download, visit: {SPH_PARSE_URL}", file=sys.stderr)
            print(f"  And paste the share link", file=sys.stderr)
    ...
    if video_channels:
        md_content += f"视频号内容：{len(video_channels)} 个\n"
    md_content += f"\n---\n\n{body}\n"
    ...
    if sph_links:
        result["video_channel_links"] = [sph["url"] for sph in sph_links]
        result["parse_service"] = SPH_PARSE_URL
